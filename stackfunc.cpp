...
int hashStack(Stack* stk)
    {
    hash_t hashArr = hashCount((size_t*)((char*)stk->dataptr - 8), stk->capacity * sizeof(elem_t));
...
int verifyStack(Stack* stk)
    ...
    if (hashCount((size_t*)((char*)stk->dataptr - 8), stk->capacity * sizeof(elem_t)) != stk->arrayhash) {error |= ArrayHashWrong;}
    return error;
    }
